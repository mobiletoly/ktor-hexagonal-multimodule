...
import ports.required.StartsNewTransaction
    ...
    companion object {
        ...
        private fun initApp(application: Application) {
            ...
            application.install(Koin) {
                SLF4JLogger()
                properties(mapOf("envConfig" to config))
                properties(mapOf("application" to application))
                modules(envTestModule, adapterModule, domainModule)
            }
        }
    }
}
